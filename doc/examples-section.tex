...
The the previous script can be found in the share/example/MPI directory in your tracing package directory. Also copy the extrae.xml file from the share/example/MPI directory instrumentation package to the current directory. This file is used to configure the behavior of the instrumentation package (there is more information about the XML file on chapter \ref{cha:XML}). The $\$\ast$ executes the arguments given to the script, so as you can run the instrumentation by simply adding the script in between your execution command.
...
If your system uses LoadLeveler your job script may look something like:
