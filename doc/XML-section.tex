...
There are two different mechanisms to instrument user functions. One is using DynInst support, the other is using the {\tt -finstrument-functions} when compiling and linking the application using the GNU C compiler.
...
\begin{itemize}
 ...
 \item If you use the GNU C compiler with the option {\tt -finstrument-functions} at compile and link stages, the  list attribute must point a file with a list of entries like:
...
\begin{itemize}
 ...
 \item {\tt sort-addresses}: whether to sort all addresses that refer to the source code (disabled by default).
\end{itemize}
