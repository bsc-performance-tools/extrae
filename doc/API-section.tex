...
\begin{itemize}
  ...
  \item \texttt{void defineEventType (int type, String description, long nValues, long[] values, String[] descriptionValues);}\\
  Adds a description for a given event type (through \texttt{type} and \texttt{description} parameters). The parameter \texttt{nValues} determines the size for the \texttt{values} and \texttt{descriptionValues} arrays. If \texttt{nValues} is non-zero, then it also adds the description for the given values.
