...
struct ForeignRecv_t* SearchForeignRecv (int group, int sender, int recver, int tag)
{
...
#if defined(DEBUG)
	fprintf (stdout, "DEBUG: SearchForeignRecv (group = %d, sender = %d, recver = %d, tag = %d)\n",
		group, sender, recver, tag);
#endif
	...
	if (myForeignRecvs_count != NULL && myForeignRecvs != NULL)
	{
		if (myForeignRecvs[group] != NULL)
			...
			{
#if defined(DEBUG)
				fprintf (stdout, "DEBUG: (sender = %d, recver = %d, tag = %d) vs (sender = %d, recver = %d, tag = %d)\n",
					sender, recver, tag, myForeignRecvs[group][i].sender, myForeignRecvs[group][i].recver, myForeignRecvs[group][i].tag);
#endif
