...
[
	...
	if test "${PMAPI_ENABLED}" = "yes" ; then
		echo "HWC access         : PMAPI"
		echo " + sampling? no"
	elif test "${PAPI_ENABLED}" = "yes" ; then
		echo "HWC access         : PAPI (${PAPI_HOME})"
		echo " + sampling? ${PAPI_SAMPLING_ENABLED}"
	fi
	if test "${BFD_INSTALLED}" = "yes" ; then
		echo "BFD                : ${BFD_HOME}"
	fi
	...
	echo "---------------------"
	echo "is BG/{L/P}        : ${IS_BGL_MACHINE} / ${IS_BGP_MACHINE}"
