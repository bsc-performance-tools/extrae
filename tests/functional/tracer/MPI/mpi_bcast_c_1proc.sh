...
EXTRAE_CONFIG_FILE=extrae.xml ${MPIRUN} -np 1 ./trace-ldpreload.sh ./mpi_bcast_c
...
CheckEntryInPCF ${TRACE}.pcf MPI_Bcast
...
NumberEntriesInPRV ${TRACE}.prv 50000002 7
if [[ "${?}" -ne 1 ]] ; then
	echo "There must be only one entry to MPI_Bcast"
	exit 1
fi
