...
EXTRAE_CONFIG_FILE=extrae.xml ${MPIRUN} -np 1 ./trace-ldpreload.sh ./mpi_gather_c

../../../../src/merger/mpi2prv -f TRACE.mpits -o ${TRACE}.prv

# Actual comparison
CheckEntryInPCF ${TRACE}.pcf MPI_Init
CheckEntryInPCF ${TRACE}.pcf MPI_Gather
...
NumberEntriesInPRV ${TRACE}.prv 50000002 13
if [[ "${?}" -ne 1 ]] ; then
	echo "There must be only one entry to MPI_Gather"
	exit 1
fi
