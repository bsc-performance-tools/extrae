...
EXTRAE_CONFIG_FILE=extrae.xml ${MPIRUN} -np 1 ./trace-ldpreload.sh ./mpi_ibcast_c
...
NENTRIES_IBCAST=`grep :50000002:157 ${TRACE}.prv | wc -l`
NENTRIES_WAIT=`grep :50000001:7 ${TRACE}.prv | wc -l`
...
if [[ "${NENTRIES_IBCAST}" -ne 1 ]] ; then
	echo "There must be only one entry to MPI_Ibcast"
	exit 1
fi
