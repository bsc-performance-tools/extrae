...
EXTRAE_CONFIG_FILE=extrae.xml ${MPIRUN} -np 1 ./trace-ldpreload.sh ./mpi_scatter_c

../../../../src/merger/mpi2prv -f TRACE.mpits -o ${TRACE}.prv

# Actual comparison
CheckEntryInPCF ${TRACE}.pcf MPI_Init
CheckEntryInPCF ${TRACE}.pcf MPI_Scatter
...
NumberEntriesInPRV ${TRACE}.prv 50000002 15
if [[ "${?}" -ne 1 ]] ; then
	echo "There must be only one entry to MPI_Scatter"
	exit 1
fi
