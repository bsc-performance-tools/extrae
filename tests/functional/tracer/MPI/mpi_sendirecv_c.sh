...
TRACE=mpi_sendirecv_c

EXTRAE_CONFIG_FILE=extrae.xml ${MPIRUN} -np 1 ./trace-ldpreload.sh ./mpi_sendirecv_c
...
CheckEntryInPCF ${TRACE}.pcf MPI_Send
...
NumberEntriesInPRV ${TRACE}.prv 50000001 1
if [[ "${?}" -ne 1 ]] ; then
	echo "There must be only one entry to MPI_Send"
	exit 1
fi

NumberEntriesInPRV ${TRACE}.prv 50000001 0
if [[ "${?}" -ne 3 ]] ; then
	echo "There must be three MPI p2p exits"
	exit 1
fi
