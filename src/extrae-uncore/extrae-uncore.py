                ...
                if line.startswith("Event name:"):
                    ...
                    if full_counter not in ExternalCounters:
                        ...
                        if len(matches.groups()) == 3:
                            ExternalCounters[full_counter]['driver'] = matches.group(1)
                            ExternalCounters[full_counter]['nic']    = matches.group(2)
                            ExternalCounters[full_counter]['port']   = matches.group(3)
        else:
            if ':' not in counter:
...
parser.add_argument('--dry-run', action='store_true', help='Calculates how many extra processes you will need to measure uncore counters')
parser.add_argument('xml_config_file', help='Extrae XML configuration file')
parser.add_argument('preload_tracing_library', help='Extrae tracing library that will be loaded with LD_PRELOAD')
...
if not os.path.isfile(Preload_app):
    print ("ERROR: Invalid tracing library '" + Preload_app + "'. Please choose from the ones available at @sub_LIBDIR@.")
    sys.exit(-1)
...
if i_am_master_process():
    ...
    if (len(ExternalCounters) > 0):
        ...
        cmd = Extrae_home + '/bin/papi_best_set ' + ','.join(sorted(ExternalCounters.keys()))
        result = subprocess.run(shlex.split(cmd), stdout=PIPE, stderr=PIPE)
        current_set = ""
        parsing_set = False
        Sets = []
        ...
        print ("Extrae: Will need " + str(num_readers) + " processes/node to measure uncore counters") 
        if (Dryrun):
            sys.exit(0)
...
if not Dryrun:
    ...
    if Enable_uncore_service and not Tracing_MPI: 
        for i in range(num_readers):
            ...
            if pid == 0:
                suffix = ('-'+str(i+1) if i > 0 else '')
                os.putenv('EXTRAE_CONFIG_FILE', './extrae_uncore' + suffix + '.xml')
    ...
    if i_am_master_process() and Enable_uncore_service:
        ...
        for c in ExternalCounters:
            ...
            ExternalCounters[c]['id'] = ' { REMAP:' + ' '.join(counter_ids) + ' }'
        ...
        # Merge the mpits
        if (Automerge):
            mpits_list = ['./TRACE.mpits'] + [item.replace('.sym', '.mpits') for item in uncore_sym_list]
            mpi2prv_cmd = Extrae_home + '/bin/mpi2prv ' + ' '.join("-f %s --" % item for item in mpits_list)
            ...
            print(mpi2prv_cmd)
            result = subprocess.run(shlex.split(mpi2prv_cmd), stdout=PIPE, stderr=PIPE)
