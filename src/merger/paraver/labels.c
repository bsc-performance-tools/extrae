...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $Source: /home/paraver/cvs-tools/mpitrace/fusion/src/merger/paraver/labels.c,v $
 | 
...
static char UNUSED rcsid[] = "$Id: labels.c,v 1.20 2009/05/28 13:06:55 harald Exp $";
