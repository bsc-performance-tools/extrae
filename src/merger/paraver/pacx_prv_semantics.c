			...
			if (MatchComms_Enabled(ptask, task, thread))
				if (MPI_PROC_NULL != Get_EvTarget (current_event))
				{
					if (isTaskInMyGroup (fset, Get_EvTarget(current_event)))
					{
						...
						CommunicationQueues_ExtractRecv (thread_info_partner->file, task-1, Get_EvTag (current_event), &recv_begin, &recv_end, &recv_thread, &recv_vthread, 0);
						...
						if (recv_begin == NULL || recv_end == NULL)
						{
							...
							CommunicationQueues_QueueSend (thread_info->file, thread_info->Send_Rec, current_event, position, thread, thread_info->virtual_thread, 0);
			...
			if (MatchComms_Enabled(ptask, task, thread))
				if (MPI_PROC_NULL != Get_EvTarget (thread_info->Send_Rec))
				{
					if (isTaskInMyGroup (fset, Get_EvTarget(thread_info->Send_Rec)))
					{
						...
						CommunicationQueues_ExtractRecv (thread_info_partner->file, task-1, Get_EvTag (thread_info->Send_Rec), &recv_begin, &recv_end, &recv_thread, &recv_vthread, 0);
			...
			if (MatchComms_Enabled(ptask, task, thread))
				if (MPI_PROC_NULL != Get_EvTarget (current_event))
				{
					if (isTaskInMyGroup (fset, Get_EvTarget(current_event)))
					{
						...
						CommunicationQueues_ExtractSend (thread_info_partner->file, task-1, Get_EvTag (current_event), &send_begin, &send_end, &send_position, &send_thread, &send_vthread, 0);
...
#endif
							CommunicationQueues_QueueRecv (thread_info->file, thread_info->Recv_Rec, current_event, thread, thread_info->virtual_thread, 0);
						}
			...
			if (NULL != receive)
			{
				if (MPI_PROC_NULL != Get_EvTarget(receive))
				{
					if (isTaskInMyGroup (fset, Get_EvTarget(receive)))
					{
						...
						CommunicationQueues_ExtractSend (thread_info_partner->file, task-1, Get_EvTag (receive), &send_begin, &send_end, &send_position, &send_thread, &send_vthread, 0);
...
#endif
							CommunicationQueues_QueueRecv (thread_info->file, current_event, receive, thread, thread_info->virtual_thread, 0);
						}
						else if (NULL != send_begin && NULL != send_end)
						{
#if defined(DEBUG)
			...
			if (MPI_PROC_NULL != Get_EvTarget (current_event))
			{
				if (isTaskInMyGroup (fset, Get_EvTarget(current_event)))
				{
					...
					if (recv_begin == NULL || recv_end == NULL)
					{
						off_t position;
						position = WriteFileBuffer_getPosition (obj_table[ptask-1].tasks[task-1].threads[thread-1].file->wfb);
						CommunicationQueues_QueueSend (thread_info->file, current_event, current_event, position, thread, thread_info->virtual_thread, 0);
						trace_paraver_unmatched_communication (1, ptask, task, thread, thread_info->virtual_thread, current_time, Get_EvTime(current_event), 1, ptask, Get_EvTarget(current_event)+1, 1, Get_EvSize(current_event), Get_EvTag(current_event));
					}
					else
