...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $Source: /home/paraver/cvs-tools/mpitrace/fusion/src/merger/paraver/trt_prv_semantics.c,v $
 | 
...
static char UNUSED rcsid[] = "$Id: trt_prv_semantics.c,v 1.3 2009/05/28 14:40:44 harald Exp $";
