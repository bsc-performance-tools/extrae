...
#error I am not maintaining this code replication.  --Glenn

#include "gm_debug_lanai_struct.h"

#if GM_OS_LINUX
#include <asm/page.h>
#define GM_CONSTANT_PAGE_SIZE PAGE_SIZE
#endif

#if !GM_CONSTANT_PAGE_SIZE
#warning GM_CONSTANT_PAGE_SIZE not defined properly

#if GM_CPU_sparc
#define GM_CONSTANT_PAGE_SIZE 8192
#endif
...
#if GM_CPU_x86
#define GM_CONSTANT_PAGE_SIZE 4096
#endif
...
#if GM_CPU_mips
#define GM_CONSTANT_PAGE_SIZE 16384
#endif
...
/* A plain copy+byte-swapping would be sufficient if pointers were the same size, but I want to use it for the Alpha 
  so I have to create new pseudo type to mimic the lanai ones.

   this file is constructed as follows (change in gm_types need to be copied here):

   structure where 32 bt field are replace by host pointers:
    gm_lanai_globals  -> gm_dump_lanai_globals
    gm_port_unprotected_lanai_side _PORT par gm_dump (replace the #ifdef lanai3 + GM_PAGE_LEN by PAGE_SIZE)
...
typedef struct gm_dump_port_unprotected_lanai_side
{
  ...
  gm_host_send_token_t send_token_queue[GM_NUM_SEND_QUEUE_SLOTS];
  /* 8 */
  /* Extra slot for wraparound. */
  gm_host_recv_token_t recv_token_queue[GM_NUM_RECV_TOKEN_QUEUE_SLOTS + 1];
  gm_u8_t padding[GM_CONSTANT_PAGE_SIZE
                 - (GM_NUM_SEND_QUEUE_SLOTS * sizeof (gm_host_send_token_t)
...
typedef union gm_dump_send_token
  {
    ...
    struct gm_dump_st_reliable
    {
      ...
      gm_up_t orig_ptr;
      gm_up_t send_ptr;
#if GM_ENABLE_FAST_SMALL_SEND
      gm_lp_t data;
#endif
    } reliable;
    struct gm_dump_st_put_get
    {
      union gm_dump_send_token *next;
      GM_SEND_TOKEN_TYPE_8 (type);
      gm_s8_t size;		/* HACK */
...
typedef struct gm_dump_connection
{
  ...
  /* 8 */
  struct gm_dump_subport * first_active_send_port;	/* may be nonzero even if
						   inactive */
...
typedef struct gm_dump_port_protected_lanai_side
  {
    gm_host_send_token_t *send_token_queue_slot;	/*lanai addr */
...
typedef struct gm_dump_lanai_globals
  {
    ...
    /*_gm_recv_t recv_token_dma_stage;*/
    /* 8 */
    /*_gm_ethernet_recv_t ethernet_recv_event_dma_stage;*/
    /* 8 */
    ...
    gm_s8_lp_t lzero;
    /* 8 */
    gm_s8_t log[GM_ROUNDUP (u32, GM_LOG_LEN, 8)];
    /* 8 */
