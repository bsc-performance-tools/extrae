...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/trunk/src/tracer/trace_macros.h $
 | @last_commit: $Date: 2011-08-22 16:30:24 +0200 (dl, 22 ago 2011) $
 | @version:     $Revision: 717 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */

#ifndef TRACE_MACROS_PACX_H_INCLUDED
#define TRACE_MACROS_PACX_H_INCLUDED

#define TRACE_PACX_CALLER_IS_ENABLED (Trace_Caller_Enabled[CALLER_MPI])

#define TRACE_PACX_CALLER(evttime,evtvalue,offset)    \
{                                                    \
	if ( ( TRACE_PACX_CALLER_IS_ENABLED ) &&          \
...
#define TRACE_PACXINITEV(evttime,evttype,evtvalue,evttarget,evtsize,evttag,evtcomm,evtaux) \
{                                                           \
	event_t evt;                                              \
	...
	if (evtvalue == EVT_END)                                  \
	{                                                         \
		last_pacx_exit_time = evt.time;                          \
	}                                                         \
}

#define TRACE_PACXEVENT(evttime,evttype,evtvalue,evttarget,evtsize,evttag,evtcomm,evtaux) \
{                                                                                    \
	int thread_id = THREADID;                                                          \
	...
	{                                                                                  \
		BURSTS_MODE_TRACE_PACXEVENT(thread_id, current_time, evtvalue, FOUR_CALLS_AGO);   \
	}                                                                                  \
	else                                                                               \
	{                                                                                  \
		NORMAL_MODE_TRACE_PACXEVENT(thread_id,                                            \
		                           current_time,                                         \
		                           evttype,                                              \
		                           evtvalue,                                             \
		                           evttarget,                                            \
	...
	{                                                                                  \
		...
		last_pacx_begin_time = current_time;                                              \
                                                                                     \
	}                                                                                  \
	...
	{                                                                                  \
		...
		last_pacx_exit_time = current_time;                                               \
		Elapsed_Time_In_PACX += last_pacx_exit_time - last_pacx_begin_time;                 \
	}                                                                                  \
}

#define NORMAL_MODE_TRACE_PACXEVENT(thread_id,evttime,evttype,evtvalue,evttarget,evtsize,evttag,evtcomm,evtaux,offset) \
{                                                               \
	event_t evt;                                                  \
	int traceja_event = 0;                                        \
                                                                \
	if (tracejant && tracejant_pacx)                              \
	{                                                             \
		...
		{                                                           \
			...
			HARDWARE_COUNTERS_READ(thread_id, evt, TRACING_HWC_PACX); \
			...
			BUFFER_INSERT(thread_id, TRACING_BUFFER(thread_id), evt); \
			TRACE_PACX_CALLER (evt.time,evtvalue,offset)               \
		}                                                           \
	}                                                             \
}
...
# define BURSTS_MODE_TRACE_PACXEVENT(thread_id, evttime, evtvalue, offset)  \
{ \
	event_t burst_begin, burst_end;           \
	burst_begin.time = last_pacx_exit_time;    \
	...
	if (evtvalue == EVT_BEGIN)                \
	{ \
		if ((burst_end.time - last_pacx_exit_time) > MINIMUM_BURST_DURATION) \
		{ \
	...
	{ \
		/* Accumulate counters from every PACX - in sampling mode it performs a simple read */ \
...
{ \
	...
	{ \
		...
		{ \
			...
			{ \
				/* This happens once when the tracing mode changes */ \
				/* from Normal to CPU Bursts, and after PACXINIT_EV */ \
...
/***
Macro TRACE_PACXEVENT_NOHWC is used to trace: IRECVED_EV, PERSISTENT_REQ_EV.
...
***/
#define TRACE_PACXEVENT_NOHWC(evttime,evttype,evtvalue,evttarget,evtsize,evttag,evtcomm,evtaux) \
{                                                         \
   if (CURRENT_TRACE_MODE(THREADID) != TRACE_MODE_BURSTS) \
   {                                                      \
      REAL_TRACE_PACXEVENT_NOHWC(evttime,                  \
                                evttype,                  \
                                evtvalue,                 \
                                evttarget,                \
                                evtsize,                  \
...
/***
Macro FORCE_TRACE_PACXEVENT is used to trace: Communicator definitions.
While in CPU Bursts tracing mode we still trace these events, since we may change later on to normal mode.
***/
#define FORCE_TRACE_PACXEVENT(evttime,evttype,evtvalue,evttarget,evtsize,evttag,evtcomm,evtaux) \
{                                       \
   REAL_FORCE_TRACE_PACXEVENT(evttime,   \
                             evttype,   \
                             evtvalue,  \
                             evttarget, \
                             evtsize,   \
...
#define REAL_TRACE_PACXEVENT_NOHWC(evttime,evttype,evtvalue,evttarget,evtsize,evttag,evtcomm,evtaux) \
{                                                               \
	event_t evt;                                                  \
	int traceja_event = 0;                                        \
	int thread_id = THREADID;                                     \
...
#define REAL_FORCE_TRACE_PACXEVENT(evttime,evttype,evtvalue,evttarget,evtsize,evttag,evtcomm,evtaux) \
{                                                             \
	event_t evt;                                              \
	int thread_id = THREADID;                                 \
                                                              \
...
#endif /* TRACE_MACROS_PACX_H_INCLUDED */
