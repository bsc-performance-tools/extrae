...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $Source: /home/paraver/cvs-tools/mpitrace/fusion/src/tracer/wrappers/MPI/mpi_wrapper.c,v $
 | 
 | @last_commit: $Date: 2009/09/23 16:56:11 $
...
static char UNUSED rcsid[] = "$Id: mpi_wrapper.c,v 1.58 2009/09/23 16:56:11 harald Exp $";
...
void CheckGlobalOpsTracingIntervals (void)
{
	...
	result = GlobalOp_Changes_Trace_Status (NumOpsGlobals);
	if (result == SHUTDOWN)
...
#warning "Aixo es pot millorar"
  ...
  else if (p_request->tipus == ISEND_EV)
  {
     P2P_Bytes_Sent += size;
  }
   ...
   *   aux: request id
   */
  TRACE_MPIEVENT_NOHWC (temps, PERSIST_REQ_EV, p_request->tipus, src_world,
...
/* Aquest codi nomes el volem per traceig sequencial i per mpi_init de fortran */
{
	...
	if (getenv ("MPTRACE_CONFIG_FILE") != NULL)
		...
		config_file = MPI_Distribute_XML_File (TASKID, NumOfTasks, getenv ("MPTRACE_CONFIG_FILE"));
	else
		config_file = NULL;
	...
	Trace_MPI_Communicator (COMM_CREATE_EV, MPI_COMM_WORLD);
	Trace_MPI_Communicator (COMM_CREATE_EV, MPI_COMM_SELF);
}
/*
#endif
*/
...
/* Aquest codi nomes el volem per traceig sequencial i per mpi_init de fortran */
{
	...
	if (*required == MPI_THREAD_MULTIPLE || *required == MPI_THREAD_SERIALIZED)
		fprintf (stderr, "mpitrace: WARNING! Instrumentation library does not support MPI_THREAD_MULTIPLE and MPI_THREAD_SERIALIZED modes\n");
...
void PMPI_Finalize_Wrapper (MPI_Fint *ierror)
{
	...
	TRACE_MPIEVENT (TIME, FINALIZE_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);

	TRACE_MYRINET_HWC();

	TRACE_MPIEVENT (TIME, FINALIZE_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
	...
	MPI_Fint *dest, MPI_Fint *tag, MPI_Fint *comm, MPI_Fint *ierror)
{
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, BSEND_EV, EVT_BEGIN, receiver, size, *tag, c, EMPTY);
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, BSEND_EV, EVT_END, receiver, size, *tag, c, EMPTY);
}
	...
	MPI_Fint *dest, MPI_Fint *tag, MPI_Fint *comm, MPI_Fint *ierror)
{
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, SSEND_EV, EVT_BEGIN, receiver, size, *tag, c, EMPTY);
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, SSEND_EV, EVT_END, receiver, size, *tag, c, EMPTY);
}
	...
	MPI_Fint *dest, MPI_Fint *tag, MPI_Fint *comm, MPI_Fint *ierror)
{
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, RSEND_EV, EVT_BEGIN, receiver, size, *tag, c, EMPTY);
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, RSEND_EV, EVT_END, receiver, size, *tag, c, EMPTY);
}
	...
	MPI_Fint *dest, MPI_Fint *tag, MPI_Fint *comm, MPI_Fint *ierror)
{
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, SEND_EV, EVT_BEGIN, receiver, size, *tag, c, EMPTY);
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, SEND_EV, EVT_END, receiver, size, *tag, c, EMPTY);
}
	...
	MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, IBSEND_EV, EVT_BEGIN, receiver, size, *tag, c, EMPTY);
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, IBSEND_EV, EVT_END, receiver, size, *tag, c, EMPTY);
}
	...
	MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, ISEND_EV, EVT_BEGIN, receiver, size, *tag, c, EMPTY);
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, ISEND_EV, EVT_END, receiver, size, *tag, c, EMPTY);
}
	...
	MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, ISSEND_EV, EVT_BEGIN, receiver, size, *tag, c, EMPTY);
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, ISSEND_EV, EVT_END, receiver, size, *tag, c, EMPTY);
}
	...
	MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, IRSEND_EV, EVT_BEGIN, receiver, size, *tag, c, EMPTY);
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, IRSEND_EV, EVT_END, receiver, size, *tag, c, EMPTY);
}
	...
	MPI_Fint *ierror)
{
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, RECV_EV, EVT_END, src_world, size, sended_tag, c, EMPTY);
}
	...
	MPI_Fint *ierror)
{
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, IRECV_EV, EVT_BEGIN, src_world, (*count) * size, *tag, c, EMPTY);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, IRECV_EV, EVT_END, src_world, (*count) * size, *tag, c, hash_req.key);
}
	...
	MPI_Fint *ierror)
{
   ...
   *   aux : root rank
   */
  TRACE_MPIEVENT (TIME, REDUCE_EV, EVT_BEGIN, *op, size, me, c, *root);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, REDUCE_EV, EVT_END, EMPTY, EMPTY, EMPTY, c, EMPTY);
}
	...
	MPI_Fint *datatype, MPI_Fint *op, MPI_Fint *comm, MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, ALLREDUCE_EV, EVT_BEGIN, *op, size, me, c, CurrentOpGlobal);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, ALLREDUCE_EV, EVT_END, EMPTY, EMPTY, EMPTY, c,
		CurrentOpGlobal);
}
	...
	MPI_Fint *status, MPI_Fint *ierror)
{
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, PROBE_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, c, EMPTY);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, PROBE_EV, EVT_END, EMPTY, EMPTY, EMPTY, c, EMPTY);
}
	...
	MPI_Fint *flag, MPI_Fint *status, MPI_Fint *ierror)
{
	MPI_Comm c = MPI_Comm_f2c(*comm);
      ...
      *   tag : ---
      */
	TRACE_MPIEVENT (TIME, IPROBE_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, c, EMPTY);
	...
	CtoF77 (pmpi_iprobe) (source, tag, comm, flag, status, ierror);
      ...
      *   tag : ---
      */
	TRACE_MPIEVENT (TIME, IPROBE_EV, EVT_END, EMPTY, EMPTY, EMPTY, c, EMPTY);
}
	...
	MPI_Fint *flag, MPI_Fint *status, MPI_Fint *ierror)
{
	...
	if (tracejant_mpi)
  {
    if (*flag)
    {
       ...
       */
      if (IProbe_Software_Counter != 0) {
        TRACE_EVENT (begin_time, TIME_OUTSIDE_IPROBES_EV, elapsed_time_outside_iprobes);
        TRACE_EVENT (begin_time, IPROBE_COUNTER_EV, IProbe_Software_Counter);
      }
      TRACE_MPIEVENT (begin_time, IPROBE_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, c, EMPTY);
      ...
      *   tag : ---
      */
      TRACE_MPIEVENT (end_time, IPROBE_EV, EVT_END, EMPTY, EMPTY, EMPTY, c, EMPTY);
    ...
    else
    {
      if (IProbe_Software_Counter == 0)
      {
        ...
        TRACE_EVENTANDCOUNTERS (begin_time, IPROBE_COUNTER_EV, 0, TRUE);
      }
...
void PMPI_Barrier_Wrapper (MPI_Fint *comm, MPI_Fint *ierror)
{
  ...
  {
    TRACE_MPIEVENT (TIME, BARRIER_EV, EVT_BEGIN, EMPTY, EMPTY, me, c,
                    CurrentOpGlobal);
  }
#else
  TRACE_MPIEVENT (TIME, BARRIER_EV, EVT_BEGIN, EMPTY, EMPTY, me, c,
  ...
  if (!BGL_disable_barrier_inside)
  {
    TRACE_MPIEVENT (TIME, BARRIER_EV, EVT_END, EMPTY, EMPTY, EMPTY, c,
                    CurrentOpGlobal);
  }
#else
  TRACE_MPIEVENT (TIME, BARRIER_EV, EVT_END, EMPTY, EMPTY, EMPTY, c,
...
void PMPI_Cancel_Wrapper (MPI_Fint *request, MPI_Fint *ierror)
{
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, CANCEL_EV, EVT_BEGIN, req, EMPTY, EMPTY, EMPTY,
                  EMPTY);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, CANCEL_EV, EVT_END, req, EMPTY, EMPTY, EMPTY, EMPTY);
}
	...
	MPI_Fint *ierror)
{
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, TEST_EV, EVT_BEGIN, *request, EMPTY, EMPTY, EMPTY,
                  EMPTY);
	...
	MPI_Fint *ierror)
{
  ...
  if (*flag && ((hash_req = hash_search (&requests, req)) != NULL))
  {
    ...
    if (Test_Software_Counter != 0) {
			TRACE_EVENT (temps_inicial, TEST_COUNTER_EV, Test_Software_Counter);
    }
    TRACE_MPIEVENT (temps_inicial, TEST_EV, EVT_BEGIN, *request, EMPTY, EMPTY, EMPTY, EMPTY);
    ...
    if (Test_Software_Counter == 0) {
      ...
      TRACE_EVENTANDCOUNTERS    (temps_inicial, TEST_COUNTER_EV, 0, TRUE);
    }
	...
	MPI_Fint array_of_statuses[][SIZEOF_MPI_STATUS], MPI_Fint * ierror)
{
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, WAITALL_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
	...
	MPI_Fint *index, MPI_Fint *status, MPI_Fint *ierror)
{
  ...
  TRACE_MPIEVENT (TIME, WAITANY_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
	...
	MPI_Fint array_of_statuses[][SIZEOF_MPI_STATUS], MPI_Fint *ierror)
{
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, WAITSOME_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
  /*
   * Arreglar-ho millor de cara a OMP
   */
	...
	MPI_Fint *root, MPI_Fint *comm, MPI_Fint *ierror)
{
  ...
  TRACE_MPIEVENT (TIME, BCAST_EV, EVT_BEGIN, *root, size, me, c, 
  	CurrentOpGlobal);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, BCAST_EV, EVT_END, EMPTY, EMPTY, EMPTY, c,
  	CurrentOpGlobal);
	...
	MPI_Fint *comm, MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, ALLTOALL_EV, EVT_BEGIN, *recvcount * recvsize,
                  *sendcount * sendsize, me, c, CurrentOpGlobal);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, ALLTOALL_EV, EVT_END, EMPTY, EMPTY, EMPTY, c,
                  CurrentOpGlobal);
}
	...
	MPI_Fint *rdispls, MPI_Fint *recvtype,	MPI_Fint *comm, MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, ALLTOALLV_EV, EVT_BEGIN, recvsize * recvc,
                  sendsize * sendc, me, c, CurrentOpGlobal);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, ALLTOALLV_EV, EVT_END, EMPTY, EMPTY, EMPTY, c,
                  CurrentOpGlobal);
}
	...
	MPI_Fint *comm, MPI_Fint *ierror)
{
   ...
   *   aux : bytes received
   */
  TRACE_MPIEVENT (TIME, ALLGATHER_EV, EVT_BEGIN, EMPTY, *sendcount * sendsize,
                  me, c, *recvcount * recvsize * nprocs);

  CtoF77 (pmpi_allgather) (sendbuf, sendcount, sendtype,
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, ALLGATHER_EV, EVT_END, EMPTY, EMPTY, EMPTY, c, EMPTY);
}
	...
	MPI_Fint *recvtype, MPI_Fint *comm, MPI_Fint *ierror)
{
   ...
   *   aux : bytes received
   */
  TRACE_MPIEVENT (TIME, ALLGATHERV_EV, EVT_BEGIN, EMPTY,
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, ALLGATHERV_EV, EVT_END, EMPTY, EMPTY, EMPTY, c, EMPTY);
}
	...
	MPI_Fint *root, MPI_Fint *comm, MPI_Fint *ierror)
{
  ...
  if (me == *root)
  {
    TRACE_MPIEVENT (TIME, GATHER_EV, EVT_BEGIN, *root, *sendcount * sendsize,
                    me, c, *recvcount * recvsize * nprocs);
  }
  else
  {
  ...
  CtoF77 (pmpi_gather) (sendbuf, sendcount, sendtype,
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, GATHER_EV, EVT_END, EMPTY, EMPTY, EMPTY, c, EMPTY);
}
	...
	MPI_Fint *recvtype, MPI_Fint *root, MPI_Fint *comm, MPI_Fint *ierror)
{
  ...
  if (me == *root)
  {
    TRACE_MPIEVENT (TIME, GATHERV_EV, EVT_BEGIN, *root, *sendcount * sendsize,
                    me, c, recvsize * recvc);
  }
  else
  {
  ...
  CtoF77 (pmpi_gatherv) (sendbuf, sendcount, sendtype,
                         recvbuf, recvcount, displs, recvtype,
                         root, comm, ierror);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, GATHERV_EV, EVT_END, EMPTY, EMPTY, EMPTY, c, EMPTY);
}
	...
	MPI_Fint *root, MPI_Fint *comm, MPI_Fint *ierror)
{
  ...
  if (me == *root)
  {
    TRACE_MPIEVENT (TIME, SCATTER_EV, EVT_BEGIN, *root,
                    *sendcount * sendsize * nprocs, me, c,
                    *recvcount * recvsize);
  }
  else
  {
    TRACE_MPIEVENT (TIME, SCATTER_EV, EVT_BEGIN, *root, 0, me, c,
                    *recvcount * recvsize);
  }
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, SCATTER_EV, EVT_END, EMPTY, EMPTY, EMPTY, c, EMPTY);
}
	...
	MPI_Fint *recvtype, MPI_Fint *root, MPI_Fint *comm, MPI_Fint *ierror)
{
  ...
  if (me == *root)
  {
    TRACE_MPIEVENT (TIME, SCATTERV_EV, EVT_BEGIN, *root, sendsize * sendc, me,
                    c, *recvcount * recvsize);
  }
  else
  {
    TRACE_MPIEVENT (TIME, SCATTERV_EV, EVT_BEGIN, *root, 0, me, c,
                    *recvcount * recvsize);
  }
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, SCATTERV_EV, EVT_END, EMPTY, EMPTY, EMPTY, c, EMPTY);
}
...
{
  TRACE_MPIEVENT (TIME, COMM_RANK_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
  CtoF77 (pmpi_comm_rank) (comm, rank, ierror);
  TRACE_MPIEVENT (TIME, COMM_RANK_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
}
...
{
  TRACE_MPIEVENT (TIME, COMM_SIZE_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
  CtoF77 (pmpi_comm_size) (comm, size, ierror);
  TRACE_MPIEVENT (TIME, COMM_SIZE_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
}
	...
	MPI_Fint *newcomm, MPI_Fint *ierror)
{
	...
	{	
		...
		Trace_MPI_Communicator (COMM_CREATE_EV, comm_id);
	}
}
	...
	MPI_Fint *ierror)
{
	...
	{
		MPI_Comm comm_id = MPI_Comm_f2c (*newcomm);
		Trace_MPI_Communicator (COMM_DUP_EV, comm_id);
	}
}
	...
	MPI_Fint *newcomm, MPI_Fint *ierror)
{
	...
	{
		MPI_Comm comm_id = MPI_Comm_f2c (*newcomm);
		Trace_MPI_Communicator (COMM_SPLIT_EV, comm_id);
	}
}
	...
	MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, REDUCESCAT_EV, EVT_BEGIN, *op, size, me, c, EMPTY);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, REDUCESCAT_EV, EVT_END, EMPTY, EMPTY, EMPTY, c, EMPTY);
}
	...
	MPI_Fint *datatype, MPI_Fint *op, MPI_Fint *comm, MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, SCAN_EV, EVT_BEGIN, *op, *count * size, me, c, CurrentOpGlobal);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, SCAN_EV, EVT_END, EMPTY, EMPTY, EMPTY, c, CurrentOpGlobal);
}
...
void PMPI_Start_Wrapper (MPI_Fint *request, MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
	TRACE_MPIEVENT (temps, START_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
   ...
   *   aux : ---
   */
	TRACE_MPIEVENT (TIME, START_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
}
	...
	MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (temps, STARTALL_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, STARTALL_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
}
...
void PMPI_Request_free_Wrapper (MPI_Fint *request, MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, REQUEST_FREE_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY,
                  EMPTY, EMPTY);
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, REQUEST_FREE_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
}
	...
	MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, RECV_INIT_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
   ...
   */
	req = MPI_Request_f2c (*request);
	PR_NewRequest (IRECV_EV, buf, *count, type, *source, *tag, c, req, &PR_queue);
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, RECV_INIT_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
}
	...
	MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, SEND_INIT_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
   ...
   */
	req = MPI_Request_f2c (*request);
	PR_NewRequest (ISEND_EV, buf, *count, type, *dest, *tag, c, req, &PR_queue);
  ...
  TRACE_MPIEVENT (TIME, SEND_INIT_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
}
	...
	MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, BSEND_INIT_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
   ...
   */
	req = MPI_Request_f2c (*request);
	PR_NewRequest (IBSEND_EV, buf, *count, type, *dest, *tag, c, req, &PR_queue);
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, BSEND_INIT_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
}
	...
	MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, RSEND_INIT_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
   ...
   */
	req = MPI_Request_f2c (*request);
	PR_NewRequest (IRSEND_EV, buf, *count, type, *dest, *tag, c, req, &PR_queue);
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, RSEND_INIT_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
}
	...
	MPI_Fint *ierror)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, SSEND_INIT_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
   ...
   */
	req = MPI_Request_f2c (*request);
	PR_NewRequest (ISSEND_EV, buf, *count, type, *dest, *tag, c, req, &PR_queue);
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, SSEND_INIT_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
}
	...
	MPI_Fint *comm, MPI_Fint *status, MPI_Fint *ierr) 
{
	...
	TRACE_MPIEVENT (TIME, SENDRECV_EV, EVT_END, SourceRank, DataSize, sender_tag, c, EMPTY);
}
	...
	MPI_Fint *comm, MPI_Fint *status, MPI_Fint *ierr) 
{
	...
	TRACE_MPIEVENT (TIME, SENDRECV_REPLACE_EV, EVT_END, SourceRank, DataSize, sender_tag, c, EMPTY);
}
...
{
    TRACE_MPIEVENT (TIME, FILE_OPEN_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
    CtoF77 (pmpi_file_open) (comm, filename, amode, info, fh, len);
    TRACE_MPIEVENT (TIME, FILE_OPEN_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
}

void PMPI_File_close_Fortran_Wrapper (MPI_File *fh, MPI_Fint *ierror)
{
    TRACE_MPIEVENT (TIME, FILE_CLOSE_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
    CtoF77 (pmpi_file_close) (fh, ierror);
    TRACE_MPIEVENT (TIME, FILE_CLOSE_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
}
...
{
    TRACE_MPIEVENT (TIME, FILE_READ_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
    CtoF77 (pmpi_file_read) (fh, buf, count, datatype, status, ierror);
    TRACE_MPIEVENT (TIME, FILE_READ_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
}

void PMPI_File_read_all_Fortran_Wrapper (MPI_File *fh, void *buf, MPI_Fint *count,
	MPI_Fint *datatype, MPI_Status *status, MPI_Fint *ierror)
{
    TRACE_MPIEVENT (TIME, FILE_READ_ALL_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
    CtoF77 (pmpi_file_read_all) (fh, buf, count, datatype, status, ierror);
    TRACE_MPIEVENT (TIME, FILE_READ_ALL_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
}

void PMPI_File_write_Fortran_Wrapper (MPI_File *fh, void *buf, MPI_Fint *count,
	MPI_Fint *datatype, MPI_Status *status, MPI_Fint *ierror)
{
    TRACE_MPIEVENT (TIME, FILE_WRITE_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
    CtoF77 (pmpi_file_write) (fh, buf, count, datatype, status, ierror);
    TRACE_MPIEVENT (TIME, FILE_WRITE_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
}

void PMPI_File_write_all_Fortran_Wrapper (MPI_File *fh, void *buf, MPI_Fint *count,
	MPI_Fint *datatype, MPI_Status *status, MPI_Fint *ierror)
{
    TRACE_MPIEVENT (TIME, FILE_WRITE_ALL_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
    CtoF77 (pmpi_file_write_all) (fh, buf, count, datatype, status, ierror);
    TRACE_MPIEVENT (TIME, FILE_WRITE_ALL_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
}
...
{
    TRACE_MPIEVENT (TIME, FILE_READ_AT_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
    CtoF77 (pmpi_file_read_at) (fh, offset, buf, count, datatype, status, ierror);
    TRACE_MPIEVENT (TIME, FILE_READ_AT_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
}

void PMPI_File_read_at_all_Fortran_Wrapper (MPI_File *fh, MPI_Offset *offset, void* buf,
	MPI_Fint *count, MPI_Fint *datatype, MPI_Status *status, MPI_Fint *ierror)
{
    TRACE_MPIEVENT (TIME, FILE_READ_AT_ALL_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
    CtoF77 (pmpi_file_read_at_all) (fh, offset, buf, count, datatype, status, ierror);
    TRACE_MPIEVENT (TIME, FILE_READ_AT_ALL_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
}

void PMPI_File_write_at_Fortran_Wrapper (MPI_File *fh, MPI_Offset *offset, void* buf,
	MPI_Fint *count, MPI_Fint *datatype, MPI_Status *status, MPI_Fint *ierror)
{
    TRACE_MPIEVENT (TIME, FILE_WRITE_AT_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
    CtoF77 (pmpi_file_write_at) (fh, offset, buf, count, datatype, status, ierror);
    TRACE_MPIEVENT (TIME, FILE_WRITE_AT_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
}

void PMPI_File_write_at_all_Fortran_Wrapper (MPI_File *fh, MPI_Offset *offset, void* buf,
	MPI_Fint *count, MPI_Fint *datatype, MPI_Status *status, MPI_Fint *ierror)
{
    TRACE_MPIEVENT (TIME, FILE_WRITE_AT_ALL_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
    CtoF77 (pmpi_file_write_at_all) (fh, offset, buf, count, datatype, status, ierror);
    TRACE_MPIEVENT (TIME, FILE_WRITE_AT_ALL_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
}
                         ...
                         int tag, MPI_Comm comm)
{
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, BSEND_EV, EVT_BEGIN, receiver, size, tag, comm,
                  EMPTY);
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, BSEND_EV, EVT_END, receiver, size, tag, comm, EMPTY);
                         ...
                         int tag, MPI_Comm comm)
{
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, SSEND_EV, EVT_BEGIN, receiver, size, tag, comm,
                  EMPTY);
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, SSEND_EV, EVT_END, receiver, size, tag, comm, EMPTY);
                         ...
                         int tag, MPI_Comm comm)
{
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, RSEND_EV, EVT_BEGIN, receiver, size, tag, comm,
                  EMPTY);
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, RSEND_EV, EVT_END, receiver, size, tag, comm, EMPTY);
                        ...
                        int tag, MPI_Comm comm)
{
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, SEND_EV, EVT_BEGIN, receiver, size, tag, comm, EMPTY);
   ...
   *   tag : message tag
   */
  TRACE_MPIEVENT (TIME, SEND_EV, EVT_END, receiver, size, tag, comm, EMPTY);
                          ...
                          int tag, MPI_Comm comm, MPI_Request *request)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, IBSEND_EV, EVT_BEGIN, receiver, size, tag, comm, EMPTY);
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, IBSEND_EV, EVT_END, receiver, size, tag, comm, EMPTY);
                         ...
                         int tag, MPI_Comm comm, MPI_Request *request)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, ISEND_EV, EVT_BEGIN, receiver, size, tag, comm, EMPTY);
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, ISEND_EV, EVT_END, receiver, size, tag, comm, EMPTY);
                          ...
                          int tag, MPI_Comm comm, MPI_Request *request)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, ISSEND_EV, EVT_BEGIN, receiver, size, tag, comm, EMPTY);
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, ISSEND_EV, EVT_END, receiver, size, tag, comm, EMPTY);
                          ...
                          int tag, MPI_Comm comm, MPI_Request *request)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, IRSEND_EV, EVT_BEGIN, receiver, size, tag, comm, EMPTY);
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, IRSEND_EV, EVT_END, receiver, size, tag, comm, EMPTY);
                        ...
                        int tag, MPI_Comm comm, MPI_Status *status)
{
   ...
   *   tag : message tag
   */
	TRACE_MPIEVENT (TIME, RECV_EV, EVT_END, src_world, size, sended_tag, comm,
		EMPTY);
	...
	int source, int tag, MPI_Comm comm, MPI_Request *request)
{
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, IRECV_EV, EVT_BEGIN, src_world, count * size, tag,
                  comm, EMPTY);
                          ...
                          MPI_Datatype datatype, MPI_Op op, int root, MPI_Comm comm)
{
   ...
   *   aux : root rank
   */
	TRACE_MPIEVENT (TIME, REDUCE_EV, EVT_BEGIN, op, size, me, comm, root);
   ...
   *   tag : ---
   */
	TRACE_MPIEVENT (TIME, REDUCE_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm, EMPTY);
                             ...
                             MPI_Datatype datatype, MPI_Op op, MPI_Comm comm)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, ALLREDUCE_EV, EVT_BEGIN, op, size, me, comm,
  	CurrentOpGlobal);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, ALLREDUCE_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm,
		CurrentOpGlobal);
...
int MPI_Probe_C_Wrapper (int source, int tag, MPI_Comm comm, MPI_Status *status)
{
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, PROBE_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, comm,
                  EMPTY);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, PROBE_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm, EMPTY);
...
int Bursts_MPI_Iprobe_C_Wrapper (int source, int tag, MPI_Comm comm, int * flag, MPI_Status *status)
{
	int ierror;
      ...
      *   tag : ---
      */
	TRACE_MPIEVENT (TIME, IPROBE_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, comm, EMPTY);
	...
	ierror = PMPI_Iprobe (source, tag, comm, flag, status);
      ...
      *   tag : ---
      */
	TRACE_MPIEVENT (TIME, IPROBE_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm, EMPTY);
                          ...
                          MPI_Status *status)
{
  ...
  last_iprobe_C_exit_time = end_time;
    ...
    if (*flag)
    {
        ...
        */
      if (IProbe_C_Software_Counter != 0) {
        TRACE_EVENT (begin_time, TIME_OUTSIDE_IPROBES_EV, elapsed_time_outside_iprobes_C);
        TRACE_EVENT (begin_time, IPROBE_COUNTER_EV, IProbe_C_Software_Counter);
      }

      TRACE_MPIEVENT (begin_time, IPROBE_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, comm, EMPTY);
       ...
       *   tag : ---
       */
      TRACE_MPIEVENT (end_time, IPROBE_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm, EMPTY);
...
int MPI_Barrier_C_Wrapper (MPI_Comm comm)
{
  ...
  if (!BGL_disable_barrier_inside)
  {
    TRACE_MPIEVENT (TIME, BARRIER_EV, EVT_BEGIN, EMPTY, EMPTY, me, comm,
                    CurrentOpGlobal);
  }
#else
  TRACE_MPIEVENT (TIME, BARRIER_EV, EVT_BEGIN, EMPTY, EMPTY, me, comm, 
  ...
  if (!BGL_disable_barrier_inside)
  {
    TRACE_MPIEVENT (TIME, BARRIER_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm,
                    CurrentOpGlobal);
  }
#else
  TRACE_MPIEVENT (TIME, BARRIER_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm,
...
int MPI_Cancel_C_Wrapper (MPI_Request *request)
{
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, CANCEL_EV, EVT_BEGIN, *request, EMPTY, EMPTY, EMPTY, EMPTY);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, CANCEL_EV, EVT_END, *request, EMPTY, EMPTY, EMPTY, EMPTY);
...
int Normal_MPI_Test_C_Wrapper (MPI_Request *request, int *flag, MPI_Status *status)
{
  ...
  if (ierror == MPI_SUCCESS && *flag && ((hash_req = hash_search (&requests, req)) != NULL))
  {
    ...
    if (Test_C_Software_Counter != 0) {
       TRACE_EVENT    (temps_inicial, TEST_COUNTER_EV, Test_C_Software_Counter);
    }
    TRACE_MPIEVENT (temps_inicial, TEST_EV, EVT_BEGIN, hash_req->key, EMPTY, EMPTY, EMPTY, EMPTY);
...
int MPI_Wait_C_Wrapper (MPI_Request *request, MPI_Status *status)
{
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, WAIT_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY);
                         ...
                         MPI_Comm comm)
{
  ...
  TRACE_MPIEVENT (TIME, BCAST_EV, EVT_BEGIN, root, size, me, comm, 
  	CurrentOpGlobal);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, BCAST_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm, 
  	CurrentOpGlobal);
  ...
  void *recvbuf, int recvcount, MPI_Datatype recvtype, MPI_Comm comm)
{
   ...
   */
  TRACE_MPIEVENT (TIME, ALLTOALL_EV, EVT_BEGIN, recvcount * recvsize,
                  sendcount * sendsize, me, comm, CurrentOpGlobal);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, ALLTOALL_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm,
                  CurrentOpGlobal);
  ...
  MPI_Datatype recvtype, MPI_Comm comm)
{
   ...
   */
  TRACE_MPIEVENT (TIME, ALLTOALLV_EV, EVT_BEGIN, recvsize * recvc,
                  sendsize * sendc, me, comm, CurrentOpGlobal);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, ALLTOALLV_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm,
                  CurrentOpGlobal);
  ...
  void *recvbuf, int recvcount, MPI_Datatype recvtype, MPI_Comm comm)
{
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, ALLGATHER_EV, EVT_END, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
  ...
  void *recvbuf, int *recvcounts, int *displs, MPI_Datatype recvtype, MPI_Comm comm)
{
   ...
   *   aux : bytes received
   */
  TRACE_MPIEVENT (TIME, ALLGATHERV_EV, EVT_BEGIN, EMPTY, sendcount * sendsize,
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, ALLGATHERV_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm, EMPTY);
  ...
  void *recvbuf, int recvcount, MPI_Datatype recvtype, int root, MPI_Comm comm)
{
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, GATHER_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm, EMPTY);
  ...
  MPI_Comm comm)
{
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, GATHERV_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm, EMPTY);
  ...
  void *recvbuf, int recvcount, MPI_Datatype recvtype, int root, MPI_Comm comm)
{
  ...
  {
    TRACE_MPIEVENT (TIME, SCATTER_EV, EVT_BEGIN, root, 0, me, comm,
                    recvcount * recvsize);
  }
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, SCATTER_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm, EMPTY);
  ...
  int root, MPI_Comm comm)
{
  ...
  {
    TRACE_MPIEVENT (TIME, SCATTERV_EV, EVT_BEGIN, root, 0, me, comm,
                    recvcount * recvsize);
  }

  ret = PMPI_Scatterv (sendbuf, sendcounts, displs, sendtype,
                       recvbuf, recvcount, recvtype, root, comm);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, SCATTERV_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm, EMPTY);
...
int MPI_Comm_create_C_Wrapper (MPI_Comm comm, MPI_Group group, MPI_Comm *newcomm)
{
  ...
  if (*newcomm != MPI_COMM_NULL && ierror == MPI_SUCCESS)
    Trace_MPI_Communicator (COMM_CREATE_EV, *newcomm);
...
int MPI_Comm_dup_C_Wrapper (MPI_Comm comm, MPI_Comm *newcomm)
{
  ...
  if (*newcomm != MPI_COMM_NULL && ierror == MPI_SUCCESS)
    Trace_MPI_Communicator (COMM_DUP_EV, *newcomm);
...
int MPI_Comm_split_C_Wrapper (MPI_Comm comm, int color, int key, MPI_Comm *newcomm)
{
  ...
  if (*newcomm != MPI_COMM_NULL && ierror == MPI_SUCCESS)
    Trace_MPI_Communicator (COMM_SPLIT_EV, *newcomm);
	...
	int *recvcounts, MPI_Datatype datatype, MPI_Op op, MPI_Comm comm)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, REDUCESCAT_EV, EVT_BEGIN, op, size, me, comm, EMPTY);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, REDUCESCAT_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm, EMPTY);
  return ierror;
}
                        ...
                        MPI_Datatype datatype, MPI_Op op, MPI_Comm comm)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (TIME, SCAN_EV, EVT_BEGIN, op, count * size, me, comm,
                  CurrentOpGlobal);
   ...
   *   tag : ---
   */
  TRACE_MPIEVENT (TIME, SCAN_EV, EVT_END, EMPTY, EMPTY, EMPTY, comm, 
  	CurrentOpGlobal);
                               ...
                               int *periods, int reorder, MPI_Comm *comm_cart)
{
  ...
  if (ierror == MPI_SUCCESS && *comm_cart != MPI_COMM_NULL)
    Trace_MPI_Communicator (MPI_CART_CREATE_EV, *comm_cart);
...
int MPI_Cart_sub_C_Wrapper (MPI_Comm comm, int *remain_dims, MPI_Comm *comm_new)
{
  ...
  if (ierror == MPI_SUCCESS && *comm_new != MPI_COMM_NULL)
    Trace_MPI_Communicator (MPI_CART_SUB_EV, *comm_new);
...
int MPI_Start_C_Wrapper (MPI_Request *request)
{
   ...
   *   aux : ---
   */
  TRACE_MPIEVENT (temps, START_EV, EVT_BEGIN, EMPTY, EMPTY, EMPTY, EMPTY,
                  EMPTY);
                             ...
                             int tag, MPI_Comm comm, MPI_Request *request)
{
   ...
   * Es guarda aquesta request 
   */
	PR_NewRequest (IRECV_EV, buf, count, datatype, source, tag, comm, *request,
                 &PR_queue);
                             ...
                             int tag, MPI_Comm comm, MPI_Request *request)
{
   ...
   * Es guarda aquesta request 
   */
	PR_NewRequest (ISEND_EV, buf, count, datatype, dest, tag, comm, *request,
                 &PR_queue);
                              ...
                              int tag, MPI_Comm comm, MPI_Request *request)
{
   ...
   * Es guarda aquesta request 
   */
	PR_NewRequest (IBSEND_EV, buf, count, datatype, dest, tag, comm, *request,
                 &PR_queue);
                              ...
                              int tag, MPI_Comm comm, MPI_Request *request)
{
   ...
   * Es guarda aquesta request 
   */
	PR_NewRequest (IRSEND_EV, buf, count, datatype, dest, tag, comm, *request,
                 &PR_queue);
                              ...
                              int tag, MPI_Comm comm, MPI_Request *request)
{
	...
	*/
	 ...
	 * Es guarda aquesta request 
	 */
	PR_NewRequest (ISSEND_EV, buf, count, datatype, dest, tag, comm, *request,
                 &PR_queue);
	...
	int source, int recvtag, MPI_Comm comm, MPI_Status * status) 
{
	...
	ret = PMPI_Type_size (DataSendType, (int *) &DataSendSize);
	MPI_CHECK(ret, PMPI_Type_size);

	ret = PMPI_Type_size (DataRecvType, (int *) &DataRecvSize);
	MPI_CHECK(ret, PMPI_Type_size);
	...
	TRACE_MPIEVENT (TIME, SENDRECV_EV, EVT_END, SourceRank, DataSize, Tag, comm,
	  EMPTY);
  ...
  MPI_Status * status) 
{
	...
	TRACE_MPIEVENT (TIME, SENDRECV_REPLACE_EV, EVT_END, SourceRank, DataSize,
	  Tag, comm, EMPTY);
	...
	*/
	...
	if (newcomm != MPI_COMM_WORLD && newcomm != MPI_COMM_SELF)
	{
		...
		for (i = 0; i < num_tasks; i++)
			FORCE_TRACE_MPIEVENT (temps, RANK_CREACIO_COMM_EV, ranks_aux[i], EMPTY,
