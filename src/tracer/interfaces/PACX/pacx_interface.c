...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/mpitrace/fusion/trunk/src/tracer/interfaces/MPI/pacx_interface.c $
 | 
...
static char UNUSED rcsid[] = "$Id: pacx_interface.c 15 2009-10-29 12:06:27Z harald $";
