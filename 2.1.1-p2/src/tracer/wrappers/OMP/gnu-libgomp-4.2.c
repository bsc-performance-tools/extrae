...
static void (*GOMP_loop_init_real) (void*,long,long,long,int,long) = NULL;
...
static int gnu_libgomp_4_2_GetOpenMPHookPoints (int rank)
{
	...
	GOMP_loop_init_real =
		(void(*)(void*,long,long,long,int,long)) dlsym (RTLD_NEXT, "GOMP_loop_init");
	if (GOMP_loop_init_real == NULL && rank == 0)
		fprintf (stderr, PACKAGE_NAME": Unable to find GOMP_loop_init in DSOs!!\n");
	INC_IF_NOT_NULL(GOMP_loop_init_real,count);
...
void GOMP_loop_init (void *p1, long p2, long p3, long p4, int p5, long p6)
{
#if defined(DEBUG)
  fprintf (stderr, PACKAGE_NAME": THREAD %d GOMP_loop_init is at %p\n", THREADID, GOMP_loop_init_real);
  fprintf (stderr, PACKAGE_NAME": THREAD %d GOMP_loop_init params %p %ld %ld %ld %d %ld \n", THREADID, p1, p2, p3, p4, p5, p6);
#endif

  if (GOMP_loop_init_real != NULL)
  {
    GOMP_loop_init_real (p1, p2, p3, p4, p5, p6);
  }
  else
  {
    fprintf (stderr, PACKAGE_NAME": GOMP_loop_init is not hooked! exiting!!\n");
    exit (0);
  }
}
